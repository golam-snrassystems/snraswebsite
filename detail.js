...
async function getData() {
    ...
    async function keepAlive() {
        await firebase.database().ref('keepAlive/' + 'Y041653632892481/' + 'online').on('value', (deviceSattus) => {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            let fetchData1 = await fetch("http://34.100.201.70:8345/getMaxSize", {
                method: "GET",
                headers: {
            ...
            firebase.database().ref('data/' + 'Y041653632892481/').orderByChild('timestamp').limitToLast(5).on('child_added', (snapshot) => {
        //    firebase.database().ref('data/' + 'Y041653632892481/').orderByValue().limitToLast(50).on('child_added', (snapshot) => {
                var userValue = snapshot.val()

                // console.log(reciveData) //latest 1 record
                // console.log(userValue) //all 50 record
                // console.log("Latest Record:", snapshot.val());
                ...
                //let ammonia = Math.round(parseFloat(userValue.ammonia) * 10) / 10; // Added ammonia
                var time = userValue.time
                 var normaltime = new Date(Number(time))
                 addItemToTable(normaltime, DO, TDS, PH, TEMPERATURE) //ammonia
        ...
        /////////////////////////////////////card layout////////////////////////
         // Populate the card with the latest data
        //  console.log(snapshot.key);
        //console.log(userValue)
        // console.log(reciveData)
        //  document.getElementById("insertion-id").innerText = snapshot.key; // Assuming the key is the Insertion ID
        //  document.getElementById("insertion-id").innerText = reciveData; // Assuming the key is the Insertion ID
        //  document.getElementById("date-time").innerText = normaltime.toLocaleString();
         document.getElementById("do-value").innerText = DO + " mg/L";
         ...
         document.getElementById('bodyId').style.visibility = "visible";
            ...
            function addItemToTable(normaltime, DO, TDS, PH, TEMPERATURE) { //ammonia
                ...
                // var td2 = document.createElement('td') //insertion id
               ...
               // var td8 = document.createElement('td')
                ...
                // td2.innerHTML = srNo //insertion id
                td3.innerHTML = normaltime
                td4.innerHTML = DO
                td5.innerHTML = TDS
                td6.innerHTML = PH
                td7.innerHTML = TEMPERATURE
                //td8.innerHTML = ammonia

                // trow.appendChild(td1)
                // trow.appendChild(td2) //insertion id
...
function getGraph() {
    ...
    var phValues = []
    var tempValues = []
    //var ammoniaValues = []; // Added ammonia
    ...
    firebase.database().ref('data/' + 'Y041653632892481').orderByValue().limitToLast(50).on('child_added', (snapshot) => {
       ...
       // var ammonia = userValue.ammonia; // Added ammonia
        var time = userValue.time
        var normaltime = new Date(Number(time))
       ...
       // ammoniaValues.push(ammonia); // Added ammonia
    ...
    new Chart("myChart", {
        ...
        data: {
            ...
            // ,{
            //     data: ammoniaValues, // Added ammonia
            //     borderColor: "purple",
            //     fill: false
            // }
        ]
...
//         console.log("inside the updateCommand")
//         firebase.database().ref('messages/' + 'Y041653632892481/').once('value', (snap) => {
//             let data = snap.val();
...
async function getAllData() {
    ...
    await fetch('http://34.100.201.70:8345/getMaxSize').then(response => response.json()).then(data => {
    ...
    async function loadData(page) {
        ...
        let fetchData = await fetch("http://34.100.201.70:8345/fetchData", {
            method: "POST",
        ...
        function waterData(id, dateTime, deviceId, resdo, tds, ph, temp) { //ammonia
            var tbody = document.getElementById('tbody2')
            // var firstRow = tbody.rows[0]
            var trow = document.createElement('tr')
            var td1 = document.createElement('td')
            ...
            var td6 = document.createElement('td')
            var td7 = document.createElement('td')
           // var td8 = document.createElement('td') //for ammonia
...
async function AllDataInCsv() {
    ...
    myToast.show();
    let fetchData2 = await fetch("http://34.100.201.70:8345/fetchAllRecord", {
        method: "GET",
        // Adding headers to the request
        headers: {
            "Content-type": "application/json; charset=UTF-8"
