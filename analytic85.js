...
async function realTime() {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            // let ammoniaData = []
            let realTimeXvalue = []
            firebase.database().ref('data/' + 'Y041653632892485/').orderByValue().limitToLast(10).on('child_added', (snapshot) => {
...
async function getData(data) {
    ...
    let fetchData = await fetch("http://34.100.201.70:8345/analytics85", {
        method: "POST",
...
function homePage() {
    ...
    window.location.href = "main85.html"
