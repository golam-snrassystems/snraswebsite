...
async function getData() {
    ...
    async function keepAlive() {
        await firebase.database().ref('keepAlive/' + 'Y041653632892502/' + 'online').on('value', (deviceSattus) => {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            let fetchData1 = await fetch("http://34.100.201.70:8345/getMaxSize502", {
                method: "GET",
            ...
            firebase.database().ref('data/' + 'Y041653632892502/').orderByValue().limitToLast(50).on('child_added', (snapshot) => {
...
async function getAllData() {
    ...
    await fetch('http://34.100.201.70:8345/getMaxSize502').then(response => response.json()).then(data => { totalItems = data[0]['MAX(id)'] }).catch(error => console.error(error));
    ...
    async function loadData(page) {
        ...
        let fetchData = await fetch("http://34.100.201.70:8345/fetchData502", { method: "POST", body: JSON.stringify(data), headers: { "Content-type": "application/json; charset=UTF-8" } })
...
async function AllDataInCsv() { var myToast = new bootstrap.Toast(document.getElementById('myToast'), { autohide: false }); myToast.show(); let fetchData2 = await fetch("http://34.100.201.70:8345/fetchAllRecord502", { method: "GET", headers: { "Content-type": "application/json; charset=UTF-8" } })
