...
function updateRotation(command) { firebase.database().ref('messages/' + 'Y041653632892505/').update({ message: command }) }
...
function makeChart(doValues, tdsValues, phValues, tempValues, perimeterValuesOrX, xValuesMaybe) {
	...
	if (perimeterValues) {
		try { window.perimeterChart = new Chart("perimeterChart", { type: "line", data: { labels: xValues, datasets: [{ label: 'PERIMETER', data: perimeterValues, borderColor: "#8A2BE2", fill: true, backgroundColor: 'rgba(138,43,226,0.08)', tension: 0.1 }] }, options: { legend: { display: true, padding: 10 } } }); } catch (e) { }
	}
	document.getElementById('loaderAnalytics').style.visibility = "hidden";
	let body = document.getElementById('containerIdAnaylicts'); body.style.filter = 'none'; body.style.pointerEvents = 'auto';
}
function homePage() { window.location.href = "main505.html" }
