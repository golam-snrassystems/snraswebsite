...
async function getData() {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            let fetchData1 = await fetch("http://34.100.201.70:8345/getMaxSize84", {
                method: "GET",
                headers: {
            ...
            firebase.database().ref('data/' + 'Y041653632892484/').orderByValue().limitToLast(50).on('child_added', (snapshot) => {
                ...
                let AMMONIA = userValue.ammonia; // Added this line to get the ammonia value
            ...
            function addItemToTable(normaltime, DO, TDS, PH, TEMPERATURE, ammonia) {
                ...
                td1.innerHTML = srNo
                td2.innerHTML = normaltime
                td3.innerHTML = DO
                td4.innerHTML = TDS
                td5.innerHTML = PH
                td6.innerHTML = TEMPERATURE      
                td7.innerHTML = ammonia
...
function getGraph() {
    ...
    firebase.database().ref('data/' + 'Y041653632892484').orderByValue().limitToLast(50).on('child_added', (snapshot) => {
        ...
        //var ammonia = ammoniaValues.ammonia
...
async function getAllData() {
    ...
    await fetch('http://34.100.201.70:8345/getMaxSize84').then(response => response.json()).then(data => {
    ...
    async function loadData(page) {
        ...
        let fetchData = await fetch("http://34.100.201.70:8345/fetchData84", {
            method: "POST",
        ...
        resdata.forEach(element => {
            ...
            // let ammoniaResponse = deviceId;
            // // let ammoniaData = ammoniaResponse.json();
            // console.log("TammoniaResponse: ", ammoniaResponse)
            // let ammonia = ammoniaData.ammonia; // Assuming your API returns an object with ammonia property
        });
        ...
        function waterData(id, dateTime, deviceId, resdo, tds, ph, temp, ammonia) {
            ...
            var td1 = document.createElement('td')
            var td2 = document.createElement('td')
            var td3 = document.createElement('td')  // New ammonia cell
            var td4 = document.createElement('td')
            var td5 = document.createElement('td')
            var td6 = document.createElement('td')
            var td7 = document.createElement('td')
            ...
            td5.innerHTML = ph
            td6.innerHTML = temp
            td7.innerHTML = ammonia;   // Display ammonia value


            trow.appendChild(td1)
            trow.appendChild(td2)
            // trow.appendChild(td3)
            trow.appendChild(td3); // Append ammonia cell
            trow.appendChild(td4)
            trow.appendChild(td5)
            trow.appendChild(td6)
            trow.appendChild(td7)
...
async function AllDataInCsv() {
    ...
    myToast.show();
    let fetchData2 = await fetch("http://34.100.201.70:8345/fetchAllRecord84", {
        method: "GET",
        // Adding headers to the request
        headers: {
            "Content-type": "application/json; charset=UTF-8"
