...
function updateRotation(command) {
    ...
    // console.log(command)
    firebase.database().ref('messages/' + 'Y041653632892497/').update({
        message: command
    })
}
...
async function realTime() {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            let tdsData = []
            let realTimeXvalue = []
            firebase.database().ref('data/' + 'Y041653632892497/').orderByValue().limitToLast(10).on('child_added', (snapshot) => {
...
async function getData(data) {
    let fetchData = await fetch("http://34.100.201.70:8345/analytics97", {
        method: "POST",
    ...
    console.log("-----resdata------97-----", resdata)
...
function homePage() {
    ...
    window.location.href = "main97.html"
