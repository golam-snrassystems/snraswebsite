...
async function realTime() {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            let tdsData = []
            let realTimeXvalue = []
            firebase.database().ref('data/' + 'Y041653632892481/').orderByValue().limitToLast(10).on('child_added', (snapshot) => {
                ...
                console.log("all value:", xaxis, localDate, DO, TDS, PH, TEMPERATURE)
                if (doData.length > 10) {
                    doData.shift()
                    phData.shift()
                    tempData.shift()
                ...
                let options = {
                    ...
                    // colors: ['#FF0000'], backgroundColor: 'rgba(255, 0, 0, 0.1)',
                ...
                chart.render();
                // for (let num = 0; num < doData.length; num++) {
...
async function getData(data) {
    ...
    let fetchData = await fetch("http://34.100.201.70:8345/analytics", {
        method: "POST",
