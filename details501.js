...
async function getData() {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            let fetchData1 = await fetch("http://34.100.201.70:8345/getMaxSize501", {
                method: "GET",
                headers: {
            ...
            firebase.database().ref('data/' + 'Y041653632892501/').orderByValue().limitToLast(50).on('child_added', (snapshot) => {
                ...
                addItemToTable(normaltime, DO, TDS, PH, TEMPERATURE, C02, PERIMETER)
            ...
            function addItemToTable(normaltime, DO, TDS, PH, TEMPERATURE, co2, perimeter) {
                ...
                var td9 = document.createElement('td')  //perimeter
...
async function getAllData() {
    ...
    await fetch('http://34.100.201.70:8345/getMaxSize501').then(response => response.json()).then(data => {
    ...
    async function loadData(page) {
        ...
        let fetchData = await fetch("http://34.100.201.70:8345/fetchData501", {
            method: "POST",
        ...
        resdata.forEach(element => {
            ...
            waterData(id, dateTime, deviceId, resdo, tds, ph, temp, co2, perimeter)
        });
        ...
        function waterData(id, dateTime, deviceId, resdo, tds, ph, temp, co2, perimeter) {
            var tbody = document.getElementById('tbody2')
            // var firstRow = tbody.rows[0]
            var trow = document.createElement('tr')
            var td1 = document.createElement('td')
            ...
            trow.appendChild(td9) //perimeter
            // tbody.insertBefore(trow, firstRow)
...
async function AllDataInCsv() {
    ...
    myToast.show();
    let fetchData2 = await fetch("http://34.100.201.70:8345/fetchAllRecord501", {
        method: "GET",
        // Adding headers to the request
        headers: {
            "Content-type": "application/json; charset=UTF-8"
