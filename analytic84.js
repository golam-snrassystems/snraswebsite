...
async function realTime() {
    ...
    async function keepAlive() {
        await firebase.database().ref('keepAlive/' + 'Y041653632892484/' + 'online').on('value', (deviceSattus) => {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            let ammoniaData = []
            let realTimeXvalue = []
            firebase.database().ref('data/' + 'Y041653632892484/').orderByValue().limitToLast(10).on('child_added', (snapshot) => {
...
async function getData(data) {
    ...
    let fetchData = await fetch("http://34.100.201.70:8345/analytics84", {
        method: "POST",
    ...
    resdata.map((element) => {
        ...
        // console.log(element.ammoniaValues)
    })
...
function homePage() {
    ...
    window.location.href = "main84.html"
