...
function updateRotation(command) {
    ...
    // console.log(command)
    firebase.database().ref('messages/' + 'Y041653632892492/').update({
        message: command
    })
}
...
async function SixMonth() {
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            // document.getElementById('co2chart').style.display = 'none'; //change required
            ...
            //  document.getElementById('co2canvas').style.display = 'block'; //change required
            ...
            makeChart(yValue.doValues, yValue.tdsValues, yValue.phValues, yValue.tempValues, xValue) //after them value add this , yValue.co2Values
        } else {
...
async function OneMonth() {
    ...
    document.getElementById('phcanvas').style.display = 'block';
    document.getElementById('tempcanvas').style.display = 'block';
    // document.getElementById('co2canvas').style.display = 'block';
    ...
    makeChart(yValue.doValues, yValue.tdsValues, yValue.phValues, yValue.tempValues, xValue) //, yValue.co2Values
}
...
async function realTime() {
    ...
    async function keepAlive() {
        await firebase.database().ref('keepAlive/' + 'Y041653632892492/' + 'online').on('value', (deviceSattus) => {
    ...
    auth.onAuthStateChanged(async (user) => {
        ...
        //    document.getElementById('co2canvas').style.display = 'none';
...
//  document.getElementById('co2chart').style.display = 'block';
            ...
            // let co2Data = []
            let realTimeXvalue = []
            firebase.database().ref('data/' + 'Y041653632892492/').orderByValue().limitToLast(10).on('child_added', (snapshot) => {
                ...
                //  let CO2 = Math.round(parseFloat(userValue.co2) * 10) / 10;
                ...
                console.log("all value:", localDate, DO, TDS, PH, TEMPERATURE) //, CO2
                doData.push(DO)
                phData.push(PH)
                ...
                //         {
                //             name: 'CO2',
                //             data: co2Data.slice() //creates a copy of co2Data to ensure the original data remains unchanged.
                ...
                //     },
                // };
                // let co2Chart = new ApexCharts(document.querySelector("#co2chart"), co2Options);
                // co2Chart.render();
...
async function getData(data) {
    let fetchData = await fetch("http://34.100.201.70:8345/analytics92", {
        method: "POST",
    ...
    console.log("-----resdata------92-----", resdata)
    ...
    resdata.map((element) => {
        ...
        //  co2Values.push(element.co2)

        xValues.push(element.dateTime)
    })
    return { doValues, tdsValues, phValues, tempValues, xValues } //, co2Values
}


function makeChart(doValues, tdsValues, phValues, tempValues, xValues) {  //, co2Values
    ...
    //  if (window.co2Chart) {
    //     if (window.co2Chart.controller) {
    //         window.co2Chart.controller.destroy();
    //     }
    // }
...
//    window.co2Chart = new Chart("co2Chart", {
//         type: "line",
//         data: {
//             labels: xValues,
...
function homePage() {
    ...
    window.location.href = "main92.html"
