...
function updateRotation(command) {
    ...
    // console.log(command)
    firebase.database().ref('messages/' + 'Y041653632892495/').update({
        message: command
    })
}
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            document.getElementById('tempchart').style.display = 'none';
             ...
             document.getElementById('ammoniachart').style.display = 'none';
            ...
            document.getElementById('tempcanvas').style.display = 'block';
             document.getElementById('ammoniacanvas').style.display = 'block';
            ...
            makeChart(yValue.doValues, yValue.tdsValues, yValue.phValues, yValue.tempValues, yValue.ammoniaValues, xValue)
        } else {
...
async function realTime() {
    ...
    async function keepAlive() {
        await firebase.database().ref('keepAlive/' + 'Y041653632892495/' + 'online').on('value', (deviceSattus) => {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            document.getElementById('tempcanvas').style.display = 'none';
              ...
              document.getElementById('ammoniacanvas').style.display = 'none';
            ...
            document.getElementById('tempchart').style.display = 'block';
             ...
             document.getElementById('ammoniachart').style.display = 'block';
            ...
            let tdsData = []
             let ammoniaData = []
            let realTimeXvalue = []
            firebase.database().ref('data/' + 'Y041653632892495/').orderByValue().limitToLast(10).on('child_added', (snapshot) => {
                ...
                let TEMPERATURE = Math.round(parseFloat(userValue.temperature) * 10) / 10;
                  ...
                  let AMMONIA = Math.round(parseFloat(userValue.ammonia) * 10) / 10;
                ...
                console.log("all value:", localDate, DO, TDS, PH, TEMPERATURE, AMMONIA)
                doData.push(DO)
                phData.push(PH)
                ...
                let ammoniaOptions = {
                    series: [
                        {
                            name: 'AMMONIA',
                            data: ammoniaData.slice() //creates a copy of ammoniaData to ensure the original data remains unchanged.
                    ...
                    title: {
                         text: 'AMMONIUM (mg/L)',// text: 'AMMONIA (mg/L)',
                        align: 'center',
                    },
                    markers: {
                        size: 0
                ...
                let ammoniaChart = new ApexCharts(document.querySelector("#ammoniachart"), ammoniaOptions);
                ammoniaChart.render();
                ammoniaChart.updateSeries([
                    {
                        name: 'AMMONIA',
                        data: ammoniaData
                    }
...
async function getData(data) {
    let fetchData = await fetch("http://34.100.201.70:8345/analytics95", {
        method: "POST",
    ...
    console.log("-----resdata------95-----", resdata)
    ...
    resdata.map((element) => {
        ...
        tempValues.push(element.temp)
         ...
         ammoniaValues.push(element.ammonia)

        xValues.push(element.dateTime)
    })
    return { doValues, tdsValues, phValues, tempValues, ammoniaValues, xValues }
}


function makeChart(doValues, tdsValues, phValues, tempValues, ammoniaValues, xValues) {
    ...
    if (window.ammoniaChart) {
        if (window.ammoniaChart.controller) {
            window.ammoniaChart.controller.destroy();
        }
    }
   ...
   window.ammoniaChart = new Chart("ammoniaChart", {
        type: "line",
        data: {
            labels: xValues,
...
function homePage() {
    ...
    window.location.href = "main95.html"
